...
router.get('/notes', async (ctx) => {
  ...
  //   }
  // }
  handleResponse(ctx, 200, 'success', { notes });
});
...
router.get('/notes/:id', async (ctx) => {
  ...
  if (note) {
    handleResponse(ctx, 200, 'success', { notes: note });
  } else {
    handleResponse(ctx, 404, `Cannot find note with id: ${id}`);
  }
});
...
router.post('/notes', Authorise, async (ctx) => {
  ...
  //   }
  // }
  handleResponse(ctx, 201, 'success', { note });
});
...
router.delete('/notes/:id', Authorise, async (ctx) => {
  ...
  if (note?.authorId !== ctx.state.userId) {
    ...
    // }
    // return;
    handleResponse(ctx, 403, 'Forbidden');
    return;
  }
  ...
  // }
  if (deleteResult) {
    handleResponse(ctx, 200, 'success');
  } else {
    handleResponse(ctx, 404, `Cannot find note with id: ${id}`);
  }
});
...
router.patch('/notes/:id', Authorise, async (ctx) => {
    ...
    if (note) {
      ...
      //   }
      // }
      handleResponse(ctx, 200, 'success', { note });
    } else {
      ...
      // }
      if (await NoteDbModel.getById(ctx.params.id)) {
        handleResponse(ctx, 400, 'Invalid request body');
      } else {
        handleResponse(ctx, 404, `Cannot find note with id: ${ctx.params.id}`);
      }
    }
  }
)
